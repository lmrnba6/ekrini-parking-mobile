...
export class ConnectionStatusService {
  ...
  constructor(private deviceInfo: DeviceInfo,
    ...
    this.deviceInfo.networkStatus.networkChange
      .subscribe(next => {
        ...
        this.reactFromStatusChange(!next);
      });

    this.reactFromStatusChange(true);
  ...
  getTroubleshootingMessages(connectionStatus: ConnectionStatus): any {
    ...
    if (!connectionStatus.isNetworkConnected) {
      messages = {
        title: this.translateService.instant('No internet connection'),
        subTitle: this.translateService.instant('No internet connection')
      };
    }
    return messages;
  }

  private reactFromStatusChange(displayAlert: boolean = true) {
    ...
    if (!connectionStatus.isConnected && displayAlert) {
